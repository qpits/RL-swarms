...
from environments.slime.slime_multichem import SlimeMultipleChem 
from agents.IQLearning import iql_multichem as iql

def create_agent(params: dict, l_params: dict, n_obs, n_actions, train):
    population = params['population']
    learner_population = params['learner_population']
    n_learners = sum(learner_population)
    ...
    action_dict = {
        str(ep): {
            str(ag): {
                str(ac): 0 
                for ac in range(n_actions)
            } for ag in range(population, population + n_learners)
        } for ep in range(1, episodes + 1)
    }
    ...
    if train:
        # Q-Learning
        # Q_table
        qtable = np.zeros([n_learners, n_obs, n_actions])
...
def create_logger(curdir, params, l_params, log_params, train, weights_path=None):
    ...
    from agents.utils.iql_logging import avg_reward_log, avg_cluster_log, epsilon_log, avg_neighbourhood_entropy_log, episode_log, actions_log, avg_neigh_others_count_log
    ...
    if train:
        log.loggers.extend([episode_log, avg_reward_log, avg_cluster_log, actions_log, epsilon_log, avg_neighbourhood_entropy_log, avg_neigh_others_count_log])
    else:
        log.loggers.extend([episode_log, avg_reward_log, avg_cluster_log, actions_log, avg_neighbourhood_entropy_log, avg_neigh_others_count_log])
...
def main(args):
    ...
    env = SlimeMultipleChem(args.random_seed, **params)
    if args.render:
        from environments.slime.slime_multichem import SlimeVisualizer
        v_params['N_CHEMICALS'] = env.n_chemicals
...
def check_args(args):
    ...
    if args.qtable_path is not  None:
        assert(args.qtable_path.endswith(".npy")), "[ERROR] qtable weights file  is not a .npy file"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--render", required=False, action='store_true')
